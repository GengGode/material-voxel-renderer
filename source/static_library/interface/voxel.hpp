...
template <typename T> struct voxel
{
    glm::ivec3 size;
    std::vector<T> memory;
    std::mdspan<T, std::dextents<int, 3>> view;
...
template <typename T> static inline voxel<T> make_voxel(glm::ivec3 size)
{
    ...
    vox.view = std::mdspan<T, std::dextents<int, 3>>(vox.memory.data(), size.x, size.y, size.z);
    return vox;
}
...
template <typename Archive, typename T> void serialize(Archive& ar, voxel<T>& vox)
{
    ...
    if (Archive::is_loading::value)
    {
        ...
        vox.view = std::mdspan<T, std::dextents<int, 3>>(vox.memory.data(), vox.size.x, vox.size.y, vox.size.z);
    }
